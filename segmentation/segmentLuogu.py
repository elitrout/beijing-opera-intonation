...
def convertFormat(featureFolder):
    ...
    cmdConvertFormat = 'python essentiaToWeka.py --inputFolder ' + featureFolder \
                       + ' --label luogu --clusters luogu,nonluogu'
...
def normalize(normvalueFile, arffFile, outputFolder, normFile):
    ...
    if _platform == 'darwin':
        cmdRenameClass = "sed -i '.original' 's/@ATTRIBUTE segment {luogu, nonluogu}"\
                         "/@ATTRIBUTE segment {0.000000000000000000e+00," \
                         "1.000000000000000000e+00}/' " + normFile
    elif _platform == "linux" or _platform == "linux2":
        cmdRenameClass = "sed -i 's/@ATTRIBUTE segment {luogu, nonluogu}"\
                         "/@ATTRIBUTE segment {0.000000000000000000e+00," \
                         "1.000000000000000000e+00}/' " + normFile
    else:
...
def smoothPrediction(prediction):
    ...
    # remove nonluogu part less than 2.5s
    vStart = 0
    vEnd = 0
    for i in range(predLen):
        if predSmooth[i] == 0 and predSmooth[max(i - 1, 0)] == 1:
            vEnd = i
            vLen = vEnd - vStart
            if vLen > 0 and vLen <= 10:
                predSmooth[vStart : vEnd] = [0 for j in range(vLen)]
        if predSmooth[i] == 1 and predSmooth[max(i - 1, 0)] == 0:
            vStart = i

    # remove luogu part less than 0.75s
    vStart = 0
    vEnd = 0
    for i in range(predLen):
        if predSmooth[i] == 1 and predSmooth[max(i - 1, 0)] == 0:
...
if __name__ ==  '__main__':
    ...
    outputFolder = './data/output_luogu_' + os.path.basename(inputFile)[:-4]
    ...
    VNnormvalueFile = './data/arias/merge/luoguMerge_norm_normvalue.pkl'
    ...
    VNmodelFile = './data/arias/merge/luogu.model'
    VNpredictionFile = outputFolder + '/predictionLN.txt'
    ...
    annotationGen(VNpredSmooth, outputFolder + '/segLuogu.txt', windowLength)
    annotationGen(VNprediction, outputFolder + '/predLuogu.txt', windowLength)
